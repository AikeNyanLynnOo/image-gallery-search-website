...
export function CollectionDialog({ isOpen, onClose, onSave }: any) {
  ...
  const [selectedLibraryImage, setSelectedLibraryImage] = useState<any>(null);
  ...
  const handleChange = (e: any) => {
    ...
    setCollection((prev: any) => ({ ...prev, [name]: value }));
  };

  const handleImageChange = (e: any) => {
    const file = e.target.files[0];
    if (file) {
      setCollection((prev: any) => ({ ...prev, coverImage: file }));

      // Create preview URL
      const reader = new FileReader();
      reader.onload = () => {
        setCollection((prev: any) => ({ ...prev, previewUrl: reader.result }));
        setView("form");
      };
  ...
  const getBackgroundClass = (bgId: any) => {
    return (
